...
function syncFontUI(){
  ...
  const dis=!activeField;
  fs.disabled=sz.disabled=dis;
  if(activeField){fs.value=activeField.fontFamily;
...
async function exportFilled(){
  try{
    if(!fields.length){setStatus('fill','Add at least one text box first','err');return;}
